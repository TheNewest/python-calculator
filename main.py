...
white = (255, 255, 255)
# the numbers for the calculator
    ...
    if event.type == pygame.MOUSEBUTTONDOWN:
        for button in symbols:
            if button.is_over(event.pos):
                print(button.text)
                calculator.new_operation(button.text)
        if clearButton.is_over(event.pos):
            calculator = Calculate()
...
def MOUSEOVERnumbers():
    if event.type == pygame.MOUSEBUTTONDOWN:
        for button in numbers:
            if button.is_over(event.pos):
                print(button.text)
                calculator.new_digit(button.text)
